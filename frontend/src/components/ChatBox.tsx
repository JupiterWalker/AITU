...
const ChatBox: React.FC<ChatBoxProps> = ({
  ...
  return (<div className="fixed bottom-0 left-0 w-full bg-white z-20 flex justify-center bg-gradient-to-t from-white/95 via-white/80 to-white/20 shadow-[0_-8px_32px_0_rgba(0,0,0,0.07)] px-2 pb-4 pt-2">
    <div className="w-full max-w-3xl">
      {contextPrompt && (
        <div className="mb-3 px-4 py-2 bg-white border border-gray-200 rounded-xl shadow-sm flex items-start relative">
          ...
          <div className="flex-1 text-gray-700 text-[15px]">
              <span className="font-semibold mr-1">{t('about', { defaultValue: '关于:' })}</span>
            {contextPrompt}
          </div>
          {setContextPrompt && (
            <button
      ...
      >
        <label className="mb-1 text-[15px] text-gray-600 flex items-center gap-1">
          <span className="text-lg">💬</span> {t('enterChildNode', { defaultValue: '请输入新的子节点内容' })}
        </label>
